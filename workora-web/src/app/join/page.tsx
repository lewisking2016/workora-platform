    ...
    try {
      const response = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          trade: formData.trade || 'Professional',
          role: formData.role === 'pro' ? 'worker' : 'hirer',
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            <motion.div key="step0" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col gap-8 w-full text-center">
              <div className="flex flex-col gap-2">
                <h1 className="text-2xl font-black tracking-tight text-zinc-950 dark:text-white">Choose your path</h1>
                <p className="text-zinc-500 dark:text-zinc-400 font-bold text-sm">How do you want to use Workora today?</p>
              </div>
              <div className="flex flex-col gap-4">
                <button onClick={() => { setFormData({ ...formData, role: 'client' }); setStep(2); }} className="flex items-center gap-6 p-8 rounded-[32px] border-2 border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 hover:border-[#0066FF] transition-all group shadow-sm hover:shadow-xl">
                  <div className="h-14 w-14 rounded-2xl bg-white dark:bg-zinc-800 flex items-center justify-center text-[#0066FF] shadow-sm group-hover:scale-110 transition-transform"><Users size={32} weight="duotone" /></div>
                  <div className="flex flex-col text-left">
                    <span className="font-black text-sm uppercase tracking-widest text-zinc-950 dark:text-white">Personal Account</span>
          ...
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-8 w-full">
               <button onClick={prevStep} className="flex items-center gap-2 text-zinc-400 hover:text-zinc-950 dark:hover:text-white transition-colors font-black text-[10px] uppercase tracking-widest">
                <CaretLeft size={14} weight="bold" /> Back
              </button>
              <div className="text-center">
                <h1 className="text-2xl font-black tracking-tight text-zinc-950 dark:text-white">What&apos;s your trade?</h1>
                <p className="text-zinc-500 dark:text-zinc-400 font-bold text-sm">Select the category that defines your expertise.</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {TRADES.map((t) => (
                  <button key={t.name} onClick={() => { setFormData({ ...formData, trade: t.name }); nextStep(); }} className="p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 hover:bg-white dark:hover:bg-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-600 transition-all flex flex-col gap-3 group">
                    <t.icon size={24} weight="duotone" className={`${t.color} group-hover:scale-110 transition-transform`} />
                    <div className="text-left">
                      <p className="text-[10px] font-black uppercase tracking-widest text-zinc-950 dark:text-white">{t.name}</p>
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-8 w-full">
              ...
              <div className="text-center">
                <h1 className="text-2xl font-black tracking-tight text-zinc-950 dark:text-white">Create your account</h1>
                <p className="text-zinc-500 dark:text-zinc-400 font-bold text-sm">You&apos;re one step away from the elite network.</p>
              </div>
              <div className="flex flex-col gap-4">
                <input placeholder="Full Name (First & Last)" className={`h-12 w-full rounded-xl bg-zinc-50 dark:bg-zinc-900 border px-4 outline-none transition-all font-bold text-xs dark:text-white dark:placeholder-zinc-500 ${formData.fullName && !isFullNameValid ? 'border-red-500 bg-red-50 dark:bg-red-950' : 'border-zinc-200 dark:border-zinc-800 focus:bg-white dark:focus:bg-zinc-800 focus:border-[#0066FF]'}`} value={formData.fullName} onChange={(e) => setFormData({ ...formData, fullName: e.target.value })} />
                ...
                <div className="relative flex gap-2">
                      ...
                      <AnimatePresence>
                         {showCountryPicker && (
                            <motion.div 
                              ...
                              className="absolute top-full left-0 z-50 mt-2 w-48 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-xl p-2"
                            >
                               {COUNTRIES.map(c => (
                   ...
                   <input placeholder="7xx xxx xxx" className={`h-12 flex-1 rounded-xl bg-zinc-50 dark:bg-zinc-900 border px-4 outline-none transition-all font-bold text-xs dark:text-white dark:placeholder-zinc-500 ${formData.phone && !isPhoneValid ? 'border-red-500 bg-red-50 dark:bg-red-950' : 'border-zinc-200 dark:border-zinc-800 focus:bg-white dark:focus:bg-zinc-800 focus:border-[#0066FF]'}`} value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value.replace(/\D/g, '') })} />
                </div>

                <input placeholder="Username" className="h-12 w-full rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-4 outline-none focus:bg-white dark:focus:bg-zinc-800 focus:border-[#0066FF] transition-all font-bold text-xs dark:text-white dark:placeholder-zinc-500" value={formData.username} onChange={(e) => setFormData({ ...formData, username: e.target.value })} />
                
                {/* Password Input with Visibility Toggle */}
                <div className="relative">
                   <input type={showPassword ? "text" : "password"} placeholder="Password (Min 8 chars + 1 number)" className={`h-12 w-full rounded-xl bg-zinc-50 dark:bg-zinc-900 border px-4 outline-none transition-all font-bold text-xs pr-12 dark:text-white dark:placeholder-zinc-500 ${formData.password && !isPasswordValid ? 'border-red-500 bg-red-50 dark:bg-red-950' : 'border-zinc-200 dark:border-zinc-800 focus:bg-white dark:focus:bg-zinc-800 focus:border-[#0066FF]'}`} value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} />
                   <button onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-950 dark:hover:text-white transition-colors">
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col gap-8 w-full text-center">
              <div className="flex flex-col gap-2">
                <h1 className="text-2xl font-black tracking-tight text-zinc-950 dark:text-white">Are you part of a team?</h1>
                <p className="text-zinc-500 dark:text-zinc-400 font-bold text-sm">This helps us tailor your experience and improve how you find work.</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => { setTeamType('solo'); setStep(4); }} className="p-8 rounded-[32px] border-2 border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 hover:border-[#0066FF] transition-all group flex flex-col items-center gap-4 shadow-sm hover:shadow-xl">
                  <div className="h-16 w-16 rounded-2xl bg-white dark:bg-zinc-800 flex items-center justify-center text-[#0066FF] shadow-sm group-hover:scale-110 transition-transform"><User size={32} weight="duotone" /></div>
                  <div className="text-center">
                    <p className="font-black text-sm text-zinc-950 dark:text-white">No</p>
            ...
            <motion.div key="step4" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-zinc-950/20 backdrop-blur-sm text-center">
               <div className="bg-white dark:bg-zinc-900 rounded-[40px] w-full max-w-[700px] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)] p-6 md:p-12 overflow-hidden relative">
                  <div className="text-center mb-10">
                     <h2 className="text-3xl font-black tracking-tighter text-zinc-950 dark:text-white">It&apos;s official... You&apos;re Workora&apos;s newest pro!</h2>
                     <p className="text-zinc-500 dark:text-zinc-400 font-bold text-sm mt-3 max-w-lg mx-auto leading-relaxed">Now&apos;s a great time to join Pro Plus Kickstart, our program for pros who are new to Workora. Access the benefits you need to help you get your first order faster.</p>
                  </div>
